...
function move(){
$(".bannerzmain .item1").css("display","none").eq(0).css("display","block");
$(".item i:even").addClass("cur"); 
	next++;
	if (next==$(".bannerzmain .item1").length) {
       next=0;
	}
	$(".bannerzmain .item1").eq(now).fadeOut().end().eq(next).fadeIn(function(){flag=true});
	$(".em").eq(now).css("display","block").end().eq(next).css("display","none");
	$(".item").eq(now).css("display","none").end().eq(next).css("display","block");
    now=next;
}
function moveR(){
    nextn=2*next;
    nextn++;
    if (nextn==$(".bannerzmain .bannertu").length) {
       nextn=0;
    }
    if (nextn%2==0) {
    nextR++; 
        if (nextR==$(".bannerzmain .item").length) {
           nextR=0;
        }
    $(".item").eq(nowR).css("display","none").end().eq(nextR).css("display","block");
    $(".em").eq(nowR).css("display","block").end().eq(nextR).css("display","none");
    nowR=nextR;
    }
    $(".bannerzmain .bannertu").eq(nown).fadeOut().end().eq(nextn).fadeIn(function(){flag=true});
    $(".item i").eq(nown).removeClass("cur").end().eq(nextn).addClass("cur");
    nown=nextn;
}
// function moveL(){
// 	next--;
...
// 2楼轮播
F1($(".F1 .F1xialaleft1 img"),$(".F1 .F1xanniu1"),$(".F1 .F1xialaleft"),$(".F2 .F1anniuleft"),$(".F2 .F1anniuright"))
...
F1($(".F6 .F1xialaleft1 img"),$(".F6 .F1xanniu1"),$(".F6 .F1xialaleft"),$(".F6 .F1anniuleft"),$(".F6 .F1anniuright"))
